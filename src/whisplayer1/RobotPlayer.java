...
public strictfp class RobotPlayer {
  ...
  public static void pathfinder(MapLocation targetLocation, RobotController rc)
    ...
    for (int i = 0; i < possibleMoves.length; i++) {
      if (rc.canMove(possibleMoves[i])) {
        ...
        if (
          targetLocation != null &&
          rc.adjacentLocation(possibleMoves[i]).equals(targetLocation)
        ) {
        ...
        int score =
          -20 *
          Math.abs(i - 2) -
          rc.senseRubble(rc.adjacentLocation(possibleMoves[i]));
        if (score > bestScore) {
          bestMove = possibleMoves[i];
          bestScore = score;
        }
      ...
      for (int i = 0; i < backupMoves.length; i++) {
        if (rc.canMove(backupMoves[i])) {
          int score =
            20 *
            Math.abs(i - 1) -
            rc.senseRubble(rc.adjacentLocation(backupMoves[i]));
  ...
  public static void addEnemyArchon(MapLocation loc, RobotController rc)
    throws GameActionException {
  ...
  public static void checkEnemyArchons(RobotController rc)
    throws GameActionException {
    for (int i = 63; i >= 60; i--) {
      ...
      if (rc.canSenseLocation(a)) {
        if (
          !rc.canSenseRobotAtLocation(a) ||
          rc.senseRobotAtLocation(a).getType() != RobotType.ARCHON
